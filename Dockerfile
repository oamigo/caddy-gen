...
RUN apk update && apk upgrade \
  ...
  # Install docker-gen
  && wget --quiet "https://github.com/nginx-proxy/docker-gen/releases/download/${DOCKER_GEN_VERSION}/docker-gen-alpine-linux-arm64-${DOCKER_GEN_VERSION}.tar.gz" \
  && tar -C /usr/bin -xvzf "docker-gen-alpine-linux-arm64-${DOCKER_GEN_VERSION}.tar.gz" \
  && rm "docker-gen-alpine-linux-arm64-${DOCKER_GEN_VERSION}.tar.gz" \
